...
const Gallery: React.FC = () => {
    ...
    return (
        <section id="gallery" className="py-24 bg-gray-50 dark:bg-ink-light/90 dark:backdrop-blur-sm">
            <div ref={ref} className={`container mx-auto px-6 transition-all duration-1000 ${animationClasses}`}>
                <div className="text-center mb-12">
                    <h2 className="text-4xl md:text-5xl font-playfair font-bold text-ink dark:text-gold-light">Our Work</h2>
                ...
                <div className="flex justify-center flex-wrap gap-2 mb-12">
                    {categories.map(cat => (
                        <button 
                            ...
                            className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors ${filter === cat ? 'bg-pink-blush text-ink dark:bg-gold dark:text-ink' : 'bg-white dark:bg-ink text-slate dark:text-gold hover:bg-gray-100 dark:hover:bg-gold/10'}`}
                        >
                            {cat}
                        </button>
                    ))}
                    ...
                    {filteredImages.map((image, index) => (
                        <div key={index} className="group relative overflow-hidden rounded-lg shadow-lg">
                            ...
                            <div className="absolute inset-0 bg-black/20 group-hover:bg-black/40 transition-colors"></div>
                             <div className="absolute bottom-4 left-4 bg-white/80 dark:bg-ink/80 text-ink dark:text-gold-light text-sm font-bold px-3 py-1 rounded-full">{image.category}</div>
                        </div>
                    ))}
                </div>
                 <div className="text-center mt-12">
                     <a href="https://www.instagram.com/lashesbynimsi" target="_blank" rel="noopener noreferrer" className="text-pink-blush hover:text-pink-hover font-semibold dark:text-gold dark:hover:text-gold-light">
