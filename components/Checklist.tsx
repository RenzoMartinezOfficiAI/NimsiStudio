...
const Checklist: React.FC = () => {
    ...
    return (
        <section id="checklist" className="py-24 bg-white dark:bg-surface-black">
            <div ref={ref} className={`container mx-auto px-6 transition-all duration-1000 ${animationClasses}`}>
                <div className="text-center mb-12">
                    <h2 className="text-4xl md:text-5xl font-playfair font-bold text-ink dark:text-gold">Before You Book</h2>
                    <p className="mt-4 text-lg text-slate max-w-2xl mx-auto dark:text-text-light">A quick checklist for your appointment.</p>
                </div>
                <div className="max-w-4xl mx-auto p-8 rounded-lg bg-cover bg-center" style={{backgroundImage: "url('https://picsum.photos/seed/image0/1200/800')"}}>
                    <div className="bg-white/80 dark:bg-brand-black/80 backdrop-blur-sm p-8 rounded-lg">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
                            {items.map((item, index) => (
                                <p key={index} className="text-slate dark:text-text-light flex items-center">
