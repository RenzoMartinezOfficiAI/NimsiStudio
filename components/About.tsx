...
const About: React.FC = () => {
    ...
    return (
        <section id="about" className="py-24 bg-gray-50 dark:bg-mage-black">
            <div ref={ref} className={`container mx-auto px-6 transition-all duration-1000 ${animationClasses}`}>
                <div className="text-center mb-16">
                    <h2 className="text-4xl md:text-5xl font-playfair font-bold text-ink dark:text-mage-gold">Meet Your Lash Artist</h2>
                    <p className="mt-4 text-lg text-slate max-w-2xl mx-auto dark:text-mage-text-light">Passion, precision, and a personal touch.</p>
                </div>
                
                <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
                    <div className="lg:w-1/3">
                        <div className="bg-white dark:bg-mage-surface p-4 rounded-lg shadow-lg">
                           <img src="https://i.postimg.cc/cHDPpkRS/image2.png" alt="Nimsi Martinez" className="w-full h-auto rounded-lg object-cover" />
                        </div>
                    </div>
                    <div className="lg:w-2/3">
                        <h3 className="text-3xl font-poppins font-bold text-ink dark:text-mage-gold">Nimsi Martinez</h3>
                        <p className="mt-4 text-slate dark:text-mage-text-light leading-relaxed">
                        ...
                        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                            {highlights.map(item => (
                                <div key={item.title} className="flex items-start gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-poppins font-semibold text-ink dark:text-mage-text-light">{item.title}</h4>
                                        <p className="text-sm text-slate dark:text-mage-text-medium">{item.text}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
