...
const NavLink: React.FC<{ href: string; children: React.ReactNode }> = ({ href, children }) => (
    <a href={href} className="text-sm font-medium text-slate hover:text-ink dark:text-slate-200 dark:hover:text-white transition-colors duration-200">
        {children}
    </a>
);
        ...
        <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/80 dark:bg-ink/80 backdrop-blur-lg shadow-sm' : 'bg-transparent'}`}>
            <div className="container mx-auto px-6 py-4 flex justify-between items-center">
                ...
                <div className="hidden lg:flex items-center space-x-4">
                    <button onClick={onThemeSwitch} className="p-2 rounded-full text-slate hover:bg-gray-100 dark:text-slate-200 dark:hover:bg-slate-800">
                        {currentTheme === 'light' ? '🌙' : '☀️'}
                    </button>
                    <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" className="bg-ink text-white dark:bg-white dark:text-ink font-poppins font-semibold text-sm py-2 px-5 rounded-lg hover:bg-slate-700 dark:hover:bg-gray-200 transition-colors duration-200">
                        Book Now
                    </a>
                </div>
            ...
            <div className={`lg:hidden fixed inset-0 bg-white/90 dark:bg-ink/90 backdrop-blur-xl pt-20 transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                 <nav className="flex flex-col items-center justify-center h-full space-y-8">
                    {navLinks.map(link => 
                     ...
                     <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" className="mt-8 bg-ink text-white dark:bg-white dark:text-ink font-poppins font-semibold py-3 px-8 rounded-lg hover:bg-slate-700 dark:hover:bg-gray-200 transition-colors duration-200">
                        Book Now
                    </a>
                </nav>
            </div>
